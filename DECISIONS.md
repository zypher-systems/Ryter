...
- **By:** lead
- **Decision:** Every request to an OpenRouter connection carries `reasoning: {effort}`. The default is `high` for the plan hat and the architect and `medium` for every role that acts. `[reasoning_effort]` overrides it per role, and `default` sends nothing. Other providers get no field. The crew reads the overrides from its meter.
...
- **Why:** The user's build turns kept "writing everything into the thinking block". This time the fix was checked in the TUI before release: plan, the hat prompt, then build in a fresh folder, and the app it wrote works.
