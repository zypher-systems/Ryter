...
pub struct Task {
    /// Stable id.
    pub id: String,
    /// Human title / prompt for the specialist.
    pub title: String,
    /// Status.
...
impl TaskQueue {
    ...
    pub fn apply_todo(&mut self, args: &Value) -> Result<()> {
        ...
        for (i, item) in items.iter().enumerate() {
            let (id, title, status) = parse_item(i, item);
...
fn parse_item(i: usize, item: &Value) -> (String, String, TaskStatus) {
    if let Some(s) = item.as_str() {
        return (format!("t{}", i + 1), s.to_string(), TaskStatus::Pending);
    }
    let title = item
        .get("content")
